...
public class Queen extends ChessPiece{
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        ...
        //Ход не по линии и не по диагонали
        if (toLine>7 || toLine < 0 || toColumn > 7 || toColumn < 0
                ...
                || (line != toLine && column != toColumn) && Math.abs(line - toLine) != Math.abs(column - toColumn)) {
            return false;
        }
        ...
        //Нет препятствия
        for (int i = 1; i < Math.max(Math.abs(line - toLine), Math.abs(column - toColumn)); i++) {
